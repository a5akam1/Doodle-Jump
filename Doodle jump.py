...
def check_collisions(rect_list, j):
    ...
    global player_y
    global y_change
    player_rect = pygame.Rect(player_x + 20, player_y + 60, 35, 5)  # Прямоугольник для игрока
    for platform in rect_list:
        platform_rect = pygame.Rect(platform[0], platform[1], platform[2], platform[3])  # Прямоугольник для платформы
